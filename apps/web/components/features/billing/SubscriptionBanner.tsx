...
export function SubscriptionBanner({ subscription }: Props) {
  ...
  if (status === 'PAST_DUE') {
    return (
      <Banner dir={dir} type="danger" icon={<CreditCard size={16} />}
        message={t('pastDue')} cta={t('payNow')}
  ...
  if (status === 'CANCELLED' || status === 'EXPIRED') {
    return (
      <Banner dir={dir} type="danger" icon={<AlertTriangle size={16} />}
        message={status === 'CANCELLED' ? t('cancelled') : t('expired')} cta={t('reactivate')}
        onCta={() => router.push(`/${locale}/billing`)} onDismiss={() => setDismissed(true)} />
    )
  }
  ...
  if (status === 'TRIAL' && trialEndsAt) {
    ...
    if (daysLeft <= 5 && daysLeft >= 0) {
      return (
        <Banner dir={dir} type="info" icon={<Clock size={16} />}
          message={t('trialEnding', { days: daysLeft })} cta={t('activateNow')}
          onCta={() => router.push(`/${locale}/billing`)} onDismiss={() => setDismissed(true)} />
      )
    }
  }
...
function Banner({
  type, icon, message, cta, onCta, onDismiss, dir,
}: {
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-3 border rounded-[8px] text-sm ${styles[type]}`} dir={dir}>
      {icon}
      <span className="flex-1">{message}</span>
      <button onClick={onCta} className={`text-white text-xs font-semibold px-3 py-1.5 rounded-[6px] transition-colors ${ctaStyles[type]}`}>
        {cta}
      </button>
