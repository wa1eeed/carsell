...
async function apiFetch<T>(url: string, options?: RequestInit): Promise<T> {
  ...
  if (!json.success) throw new Error(typeof json.error === 'string' ? json.error : 'حدث خطأ')
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[#0F3460]">كتالوج السيارات</h1>
...
function BrandsTab() {
  ...
  async function save() {
    ...
    try {
      ...
      await apiFetch('/api/v1/admin/catalog/brands', {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => void load()} className="p-2 hover:bg-gray-100 rounded-[8px] text-gray-400">
            <RefreshCw size={14} />
          </button>
          <button
            onClick={openCreate}
                ...
                <tr key={b.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-1">
                      <button onClick={() => openEdit(b)} className="p-1.5 hover:bg-gray-100 rounded text-gray-400 hover:text-[#0F3460]">
                        <Pencil size={14} />
                      </button>
                      <button onClick={() => void remove(b)} className="p-1.5 hover:bg-red-50 rounded text-gray-400 hover:text-red-500">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
...
function CategoriesTab() {
  ...
  const [saving, setSaving]           = useState(false)
  const [error, setError]             = useState('')
  const [form, setForm]               = useState({ brandId: '', nameAr: '', nameEn: '', bodyType: 'SEDAN' as BodyType })
  ...
  function openCreate() {
    setForm({ brandId: filterBrandId || (brands[0]?.id ?? ''), nameAr: '', nameEn: '', bodyType: 'SEDAN' })
    setEditItem(null)
    setShowForm(true)
    setError('')
  }
  ...
  function openEdit(c: Category) {
    setForm({ brandId: c.brandId, nameAr: c.nameAr, nameEn: c.nameEn, bodyType: c.bodyType })
    setEditItem(c)
    setShowForm(true)
    setError('')
  }
  ...
  async function save() {
    if (!form.brandId || !form.nameAr.trim() || !form.nameEn.trim()) {
    ...
    try {
      ...
      await apiFetch('/api/v1/admin/catalog/categories', {
        method: editItem ? 'PATCH' : 'POST',
        body: JSON.stringify(payload),
      })
      setShowForm(false)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => void loadCategories()} className="p-2 hover:bg-gray-100 rounded-[8px] text-gray-400">
            <RefreshCw size={14} />
          </button>
          <button
            onClick={openCreate}
          ...
          <table className="w-full text-sm">
            <thead className="bg-[#F8FAFC] border-b border-gray-100">
              <tr>
                ...
                <th className="text-right p-4 font-medium text-gray-500">نوع الهيكل</th>
                <th className="text-right p-4 font-medium text-gray-500">الموديلات</th>
                <th className="text-right p-4 font-medium text-gray-500">الحالة</th>
                <th className="p-4"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {categories.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-1">
                      <button onClick={() => openEdit(c)} className="p-1.5 hover:bg-gray-100 rounded text-gray-400 hover:text-[#0F3460]">
                        <Pencil size={14} />
                      </button>
                      <button onClick={() => void remove(c)} className="p-1.5 hover:bg-red-50 rounded text-gray-400 hover:text-red-500">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
...
function ModelsTab() {
  ...
  const [saving, setSaving]               = useState(false)
  const [error, setError]                 = useState('')
  const [form, setForm]                   = useState({ brandId: '', categoryId: '', name: '' })
  ...
  // Load categories for the form when form brand changes
  useEffect(() => {
  ...
  function openCreate() {
    setForm({ brandId: filterBrandId || '', categoryId: filterCatId || '', name: '' })
    setEditItem(null)
    setShowForm(true)
    setError('')
  }
  ...
  function openEdit(m: Model) {
    setForm({ brandId: m.category.brand.id, categoryId: m.categoryId, name: m.name })
    setEditItem(m)
    setShowForm(true)
    setError('')
  }
  ...
  async function save() {
    ...
    try {
      ...
      await apiFetch('/api/v1/admin/catalog/models', {
        method: editItem ? 'PATCH' : 'POST',
        body: JSON.stringify(payload),
      })
      setShowForm(false)
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex gap-2">
          <select
            value={filterBrandId}
            onChange={(e) => { setFilterBrandId(e.target.value); setFilterCatId('') }}
            className="border border-gray-200 rounded-[8px] px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#0F3460] bg-white"
          >
            <option value="">كل الماركات</option>
            {brands.map((b) => <option key={b.id} value={b.id}>{b.nameAr}</option>)}
        ...
        <div className="flex gap-2">
          <button onClick={() => void loadModels()} className="p-2 hover:bg-gray-100 rounded-[8px] text-gray-400">
            <RefreshCw size={14} />
          </button>
          <button
            onClick={openCreate}
        ...
        <div className="bg-[#F8FAFC] border border-gray-200 rounded-[12px] p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select
                value={form.brandId}
                onChange={(e) => setForm((f) => ({ ...f, brandId: e.target.value, categoryId: '' }))}
                className="w-full border border-gray-200 rounded-[8px] px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#0F3460]"
              >
                <option value="">اختر الماركة</option>
                {brands.map((b) => <option key={b.id} value={b.id}>{b.nameAr}</option>)}
            ...
            <tbody className="divide-y divide-gray-50">
              {models.map((m) => (
                <tr key={m.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-1">
                      <button onClick={() => openEdit(m)} className="p-1.5 hover:bg-gray-100 rounded text-gray-400 hover:text-[#0F3460]">
                        <Pencil size={14} />
                      </button>
                      <button onClick={() => void remove(m)} className="p-1.5 hover:bg-red-50 rounded text-gray-400 hover:text-red-500">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
